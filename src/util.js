export function parseLtvs (fullAd) {
  ...
  while (i < fullAd.length) {
    // converting first 2 bytes to decimal to get the length
    const tlen = parseInt(fullAd.substr(i, 2), 16)
    if (tlen <= 0) {
      return map
    }
    // converting the next 2 bytes to decimal to get the type
    const t = fullAd.substr(i + 2, 2)
    // converting the next len bytes to decimal to get the value
...
export const hexToBytes = (hex) => {
  return hex.match(/.{1,2}/g)
    .map((byte, idx) => parseInt(hex.substr(idx * 2, 2), 16))
}
